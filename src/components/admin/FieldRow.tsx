...
export default function FieldRow({ field, setFields }: Props) {
    ...
    const handleUpdate = async (payload: any) => {
        try {
            ...
            setFields((prev: Field[]) => prev.map(f => f.id === field.id ? { ...f, ...payload } : f))
        } catch (error) {
            console.error(error)
        }
    }
    ...
    return (
        <TableRow>
            <DialogEditField field={field} open={openEdit} onOpenChange={setOpenEdit} handleUpdate={handleUpdate} />
            <DialogDeleteField open={openDelete} onOpenChange={setOpenDelete} handleDelete={handleDelete} />
